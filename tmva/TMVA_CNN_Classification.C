...
void TMVA_CNN_Classification(std::vector<bool> opt = {1, 1, 1, 1, 1})
{
   ...
   if (writeOutputFile)
      outputFile = TFile::Open("mcbm_ring_reco.root", "RECREATE");
   ...
   ***/
   ...
   TMVA::Factory factory(
      ...
      "!V:ROC:!Silent:Color:AnalysisType=Regression:Transformations=None:!Correlations");
   ...
   **/
   ...
   loader->AddRegressionTree((TTree*)data->Get("train"), 1.0, TMVA::Types::ETreeType::kTraining);     
   ...
   //loader->AddVariablesArray("vars", imgSize);
    ...
    for(int i{} ; i < 72*32 ; i++){
        loader->AddVariable("x[" + std::to_string(i) + "]", "x_" + std::to_string(i), "" );
    }
    // or AddVariableArray
    for(int i{} ; i < 15 ; i++){
        loader->AddTarget("y[" + std::to_string(i) + "]", "y_" + std::to_string(i), "" );
    }
   ...
   loader->PrepareTrainingAndTestTree(TCut(""), "SplitMode=Random:NormMode=None:V:!Correlations:!CalcCorrelations");
   ...
   **/
   ...
   if (useKerasCNN) {
      ...
      m.AddLine("model.add(Reshape((72, 32, 1), input_shape = (2304, )))");
      ...
      m.AddLine("model.add(Dense(15, activation = 'relu')) ");
      m.AddLine("model.compile(loss = 'mse', optimizer = Adam(lr = 0.001), metrics = ['accuracy'])");
